...
def status_to_processing(order_id):
    """
    Cambiar el estado de un pedido de 'wc-on-hold' a 'wc-processing'
    Este es un endpoint de acción rápida para el botón "Procesar".
    """
    try:
        from sqlalchemy import text
        from datetime import datetime

        # Verificar que el pedido existe y tiene estado on-hold
        order_query = text("""
            SELECT status 
            FROM wpyz_wc_orders 
            WHERE id = :order_id
        ...
        if order.status != 'wc-on-hold':
            return jsonify({
                'success': False,
                'error': f'Solo se pueden procesar pedidos "En espera". (Estado actual: {order.status})'
            }), 400

        # Actualizar estado en wpyz_wc_orders
        ...
        """)
        now = datetime.utcnow()
        db.session.execute(update_order_query, {'order_id': order_id, 'now': now})

        # Sincronizar con wpyz_posts si existe (para compatibilidad con plugins legacy)
        update_posts_query = text("""
            UPDATE wpyz_posts
            ...
            WHERE ID = :order_id
        """)
        db.session.execute(update_posts_query, {'order_id': order_id, 'now': now})

        db.session.commit()

        current_app.logger.info(f"STATUS-CHANGE: Order {order_id} changed from on-hold to processing by {current_user.username}")

        return jsonify({
            'success': True,
            'message': f'Pedido {order_id} movido a "Procesando" correctamente'
        })

    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error changing status to processing for order {order_id}: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500


@bp.route('/api/trash-order/<int:order_id>', methods=['POST'])
@login_required
def trash_order(order_id):
    """
    Mover un pedido a la papelera (status = 'trash')
    """
    try:
        from sqlalchemy import text
        from datetime import datetime
        ...
        order_query = text("SELECT status FROM wpyz_wc_orders WHERE id = :order_id")
        order = db.session.execute(order_query, {'order_id': order_id}).fetchone()

        if not order:
            return jsonify({'success': False, 'error': f'Pedido {order_id} no encontrado'}), 404

        # 2. Si se pide restaurar stock, hacerlo antes de borrar
        if restore_stock:
             # Obtener items y devolver al stock
             items_query = text("""
                SELECT 
                    (SELECT meta_value FROM wpyz_woocommerce_order_itemmeta WHERE order_item_id = oi.order_item_id AND meta_key = '_qty' LIMIT 1) as qty,
                    (SELECT meta_value FROM wpyz_woocommerce_order_itemmeta WHERE order_item_id = oi.order_item_id AND meta_key = '_product_id' LIMIT 1) as product_id,
                    (SELECT meta_value FROM wpyz_woocommerce_order_itemmeta WHERE order_item_id = oi.order_item_id AND meta_key = '_variation_id' LIMIT 1) as variation_id
                FROM wpyz_woocommerce_order_items oi
                WHERE oi.order_id = :order_id AND oi.order_item_type = 'line_item'
             """)
             items = db.session.execute(items_query, {'order_id': order_id}).fetchall()

             for item in items:
                 qty = float(item[0]) if item[0] else 0
                 product_id = int(item[1]) if item[1] else 0
                 variation_id = int(item[2]) if item[2] else 0

                 target_id = variation_id if variation_id > 0 else product_id

                 if target_id > 0 and qty > 0:
                     # Actualizar stock en postmeta
                     db.session.execute(text("""
                        UPDATE wpyz_postmeta 
                        SET meta_value = CAST(meta_value AS DECIMAL(10,2)) + :qty 
                        WHERE post_id = :pid AND meta_key = '_stock'
                     """), {'qty': qty, 'pid': target_id})

                     # Cambiar status a instock si era outofstock
                     db.session.execute(text("""
                        UPDATE wpyz_postmeta 
                        SET meta_value = 'instock'
                        WHERE post_id = :pid AND meta_key = '_stock_status' AND meta_value = 'outofstock'
                     """), {'pid': target_id})

        # 3. Aplicar status 'trash'
        now = datetime.utcnow()
        db.session.execute(text("""
            UPDATE wpyz_wc_orders 
            SET status = 'trash', date_updated_gmt = :now 
            WHERE id = :order_id
        """), {'order_id': order_id, 'now': now})

        db.session.execute(text("""
            UPDATE wpyz_posts 
            SET post_status = 'trash', post_modified = :now, post_modified_gmt = :now 
            WHERE ID = :order_id
        """), {'order_id': order_id, 'now': now})

        db.session.commit()
        return jsonify({'success': True, 'message': f'Pedido {order_id} movido a la papelera'})

    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error trashing order {order_id}: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
